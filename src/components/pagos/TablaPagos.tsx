...
interface Props {
  pageData: PageResponse<PagoResponseDto> | null;
  loading: boolean;
  error: string | null;
  onPageChange: (newPage: number) => void;
  onAnular: (id: string, nombreCliente: string) => void;
}
        ...
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-200">
            {pageData?.content.length === 0 ? (
              ...
              pageData?.content.map((pago) => (
                <tr
                  key={pago.id}
                  className={`hover:bg-gray-50 transition-colors ${
                    !pago.estadoActivo ? 'opacity-50' : ''
                  }`}
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <div className="flex items-center justify-center gap-3">
                      {pago.estadoActivo ? (
                        <button
                          onClick={() => onAnular(pago.id, pago.nombreCliente)}
      ...
      {pageData && (
        <div className="flex justify-between items-center p-4 bg-white border-t border-gray-200 shrink-0">
          <div className="text-sm text-gray-700">
            Mostrando {pageData.content.length} pagos de {pageData.totalElements} en total
          </div>
